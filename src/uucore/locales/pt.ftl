...
error-file-not-found = Ficheiro ou diretório inexistente
error-invalid-argument = Argumento inválido
error-is-a-directory = { $file }: É um diretório
action-copying = a copiar
action-moving = a mover
action-removing = a remover
action-creating = a criar
action-reading = a ler
action-writing = a gravar
selinux-error-not-enabled = O SELinux não está ativado neste sistema
selinux-error-file-open-failure = falha ao abrir o ficheiro: { $error }
selinux-error-context-retrieval-failure = falha ao obter o contexto de segurança: { $error }
selinux-error-context-set-failure = falha ao definir o contexto predefinido de criação de ficheiros como '{ $context }': { $error }
selinux-error-context-conversion-failure = falha ao definir o contexto predefinido de criação de ficheiros como '{ $context }': { $error }
selinux-error-operation-not-supported = operação não apoiada
...
safe-traversal-error-invalid-fd = descritor de ficheiro inválido
checksum-no-properly-formatted = { $checksum_file }: não foram encontradas linhas de somas de verificação formatadas corretamente
checksum-no-file-verified = { $checksum_file }: nenhum ficheiro foi verificado
...
checksum-failed-open-file =
    { $count ->
        [1] Não foi possível ler { $count } ficheiro listado
       *[other] Não foi possível ler { $count } ficheiros listados
    }
checksum-error-algo-bad-format = { $file }: { $line }: linha de soma de verificação { $algo } com formatação incorreta
...
smack-error-label-set-failure = falha ao definir o contexto predefinido de criação de ficheiros como '{ $context }': { $error }
