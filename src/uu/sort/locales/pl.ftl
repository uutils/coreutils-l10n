...
sort-compress-prog-execution-failed = nie można uruchomić programu kompresującego '{ $prog }': { $error }
...
sort-invalid-field-spec = { $msg }: nieprawidłowa specyfikacja pola { $spec }
sort-invalid-count-at-start-of = nieprawidłowy wynik na początku { $string }
sort-invalid-number-at-field-start = nieprawidłowa liczba na początku pola
sort-invalid-number-after-dash = nieprawidłowa liczba po '-'
sort-invalid-number-after-dot = nieprawidłowa liczba po '.'
sort-invalid-number-after-comma = nieprawidłowa liczba po ','
sort-field-number-is-zero = numer pola wynosi zero
sort-character-offset-is-zero = przesunięcie znaku wynosi zero
sort-stray-character-field-spec = zabłąkany znak w specyfikacji pola
sort-warning-failed-to-set-locale = nie udało się ustawić ustawień regionalnych
sort-warning-simple-byte-comparison = kolejność tekstu wykonywana za pomocą prostego porównania bajtów
sort-warning-sort-rule = kolejność tekstu wykonywana za pomocą reguł sortowania „{ $locale }”
sort-warning-key-zero-width = klucz { $key } ma zerową szerokość i zostanie zignorowany
sort-warning-key-numeric-spans-fields = klucz { $key } jest numeryczny i obejmuje wiele pól
sort-warning-leading-blanks-significant = wiodące spacje są istotne w kluczu { $key }; rozważ także określenie 'b'
sort-warning-numbers-use-decimal-point = liczby używają kropki dziesiętnej '.' w tych ustawieniach regionalnych
sort-warning-options-ignored = opcje '-{ $options }' są ignorowane
sort-warning-option-ignored = opcja '-{ $option }' jest ignorowana
sort-warning-option-reverse-last-resort = opcja '-r' dotyczy tylko porównania w ostatniej instancji
sort-warning-obsolescent-key = użyto przestarzałego klucza '{ $key }'; zamiast niego należy rozważyć '-k { $replacement }'
sort-warning-separator-grouping = separator pól '{ $sep }' jest traktowany jako separator grup w liczbach
sort-warning-separator-decimal = separator pól '{ $sep }' jest traktowany jako przecinek dziesiętny w liczbach
sort-warning-separator-minus = separator pól '{ $sep }' jest traktowany jako znak minus w liczbach
sort-warning-separator-plus = separator pól '{ $sep }' jest traktowany jako znak plus w liczbach
sort-help-random-source = używa PLIKU jako źródła losowych danych
sort-read-failed = niepowodzenie odczytu: { $error }
sort-random-source-end-of-file = { $path }: koniec pliku
sort-diag-label-missing-count = tutaj powinna być liczba
sort-diag-label-zero-count = liczenie zaczyna się od 1
sort-diag-help-key-syntax = kluczem jest POLE[.ZNAK][OPCJE][,POLE[.ZNAK][OPCJE]], jak w -k2.3,4nr
