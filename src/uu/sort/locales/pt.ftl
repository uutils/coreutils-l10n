...
sort-invalid-field-spec = { $msg }: especificação de campo inválida { $spec }
sort-invalid-number-at-field-start = número inválido no início do campo
sort-invalid-number-after-dash = número inválido após '-'
sort-invalid-number-after-dot = número inválido após '. '
sort-invalid-number-after-comma = número inválido após ','
sort-field-number-is-zero = número de campo é zero
sort-warning-key-zero-width = chave { $key } tem a largura zero e será ignorada
sort-warning-key-numeric-spans-fields = chave { $key } é numérica e abrange vários campos
sort-warning-numbers-use-decimal-point = números usam '.' como um ponto decimal neste locale
...
sort-warning-failed-to-set-locale = falha ao definir locale
sort-warning-simple-byte-comparison = ordenação de texto realizada usando comparação de bytes simples
sort-warning-separator-decimal = o separador de campos '{ $sep }' é tratado como um ponto decimal em números
