...
sort-random-source-end-of-file = {$path} : fin de fichier
...
sort-diag-label-zero-count = le comptage commence à 1
sort-diag-help-key-syntax = une clé est CHAMP[.CAR][OPTS][,CHAMP[.CAR][OPTS]], comme dans -k2.3,4nr
...
sort-warning-failed-to-set-locale = échec de la définition de la locale
sort-warning-simple-byte-comparison = ordonnancement du texte effectué par simple comparaison d'octets
sort-warning-sort-rule = ordonnancement du texte effectué avec les règles de tri « {$locale} »
sort-warning-key-zero-width = la clé {$key} est de largeur nulle et sera ignorée
sort-warning-key-numeric-spans-fields = la clé {$key} est numérique et s'étend sur plusieurs champs
sort-warning-leading-blanks-significant = les espaces de début sont significatifs dans la clé {$key} ; envisagez de spécifier aussi 'b'
sort-warning-numbers-use-decimal-point = les nombres utilisent '.' comme séparateur décimal dans cette locale
sort-warning-options-ignored = les options '-{$options}' sont ignorées
sort-warning-option-ignored = l'option '-{$option}' est ignorée
sort-warning-option-reverse-last-resort = l'option '-r' ne s'applique qu'à la comparaison de dernier recours
sort-warning-obsolescent-key = clé obsolète '{$key}' utilisée ; envisagez plutôt '-k {$replacement}'
sort-warning-separator-grouping = le séparateur de champ '{$sep}' est traité comme un séparateur de groupe dans les nombres
sort-warning-separator-decimal = le séparateur de champ '{$sep}' est traité comme un séparateur décimal dans les nombres
sort-warning-separator-minus = le séparateur de champ '{$sep}' est traité comme un signe moins dans les nombres
sort-warning-separator-plus = le séparateur de champ '{$sep}' est traité comme un signe plus dans les nombres
