...
tr-warning-unescaped-backslash = 警告：在字符串结尾使用未转义的反斜杠是一种不可移植的做法
tr-after-help = 从标准输入转换、压缩和/或删除字符，并将结果写入标准输出。
tr-help-truncate-set1 = 先将字符集1截断为与字符集2相同的长度
tr-error-missing-operand = 缺少操作对象
...
tr-error-missing-char-class-name = "[::]" 中缺少字符类名称
tr-error-missing-equivalence-class-char = "[==]" 中缺少等价类字符
...
tr-error-empty-set2-when-not-truncating = 不截断 集合1 时，字符串2 不能为空
