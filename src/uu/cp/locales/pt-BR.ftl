...
cp-error-cannot-change-attribute = não é possível alterar o atributo { $dest }: O arquivo de fonte não é um arquivo comum
cp-error-cannot-stat = não é possível obter os metadados de { $source }: Arquivo ou diretório inexistente
cp-error-cannot-create-symlink = não é possível criar um link simbólico de { $dest } para { $source }
cp-error-cannot-create-hard-link = não é possível criar um link físico de { $dest } para { $source }
cp-error-omitting-directory = -r não especificado; omitindo o diretório { $dir }
cp-error-cannot-copy-directory-into-itself = não é possível copiar um diretório, { $source }, para si mesmo, { $dest }
cp-error-will-not-copy-through-symlink = não copiará { $source } através do link simbólico { $dest } recém-criado
cp-error-will-not-overwrite-just-created = não substituirá o arquivo { $dest } recém-criado pelo arquivo { $source }
cp-error-target-not-directory = destino: { $target } não é um diretório
cp-error-cannot-overwrite-directory-with-non-directory = não é possível sobrescrever o diretório { $dir } com um não-diretório
cp-error-cannot-overwrite-non-directory-with-directory = não é possível sobrescrever um não-diretório com um diretório
cp-error-with-parents-dest-must-be-dir = com a opção --parents, o destino deve ser um diretório
cp-error-not-replacing = não substituindo { $file }
