...
cp-error-not-writing-dangling-symlink = bez zapisywania przez nieaktywne dowiązanie symboliczne { $dest }
...
cp-error-selinux-set-context = nie udało się ustawić kontekstu zabezpieczeń { $path }
...
cp-error-cannot-create-regular-file = nie można utworzyć zwykłego pliku { $path }
