...
od-after-help =
    Exhibe datos en varios formatos legibles por humanos. Si se especifican múltiples
    formatos, la salida contendrá todos los formatos en el orden que aparecen en la
    ...
    Si no se especifica nombre de archivo, o es "-", se usará stdin. Tras un "--", no se
    reconocerán más opciones. Esto permite nombres de archivo que empiecen con "-".

    Si un nombre de archivo es un número válido que puede usarse como
    desplazamiento en la segunda forma, puede forzar que se reconozca
    como nombre de archivo si incluye una opción como "-j0", que solo es
    válida en la primera forma.

    RADIX es uno de o,d,x,n para octal, decimal, hexadecimal o ninguno.

    BYTES es decimal por defecto, octal si tiene prefijo "0", o hexadecimal si tiene
    prefijo "0x". Los sufijos b, KB, K, MB, M, GB, G, multiplicarán el número con
    512, 1000, 1024, 1000^2, 1024^2, 1000^3, 1024^3, 1000^2, 1024^2.

    DESPLAZAMIENTO y ETIQUETA son octal por defecto, hexadecimal si tienen
    prefijo "0x" o decimal si se agrega un sufijo ".". El sufijo "b" multiplicará con 512.
    ...
    TAMAÑO es el número de bytes que puede ser el número 1, 2, 4, 8 o 16,
        o  C, S, I, L para 1, 2, 4, 8 bytes para tipos enteros,
        o  F, D, L para 4, 8, 16 bytes para punto flotante.
    Cualquier especificación de tipo puede tener un sufijo "z", que agregará un
    volcado ASCII al final de la línea.
...
od-error-too-many-inputs = demasiadas entradas tras --traditional: { $input }
od-error-parse-failed = falló el intérprete
...
od-error-argument-too-large = argumento { $option } { $value } es demasiado grande
...
od-help-strings = cadenas de salida de al menos BYTES de caracteres gráficos. Se asume 3 cuando BYTES no está especificado.
od-help-o = 2-byte de unidades octales
od-help-capital-i = 8-byte de unidades decimales
od-help-capital-l = 8-byte de unidades decimales
od-help-i = 4-byte de unidades decimales
od-help-l = 8-byte de unidades decimales
od-help-x = 2-byte de unidades hexadecimales
od-help-h = 2-byte de unidades hexadecimales
od-help-capital-o = 4-byte de unidades octales
od-help-s = 2-byte de unidades decimales
od-help-capital-x = 4-byte de unidades hexadecimales
od-help-capital-h = 4-byte de unidades hexadecimales
od-help-e = unidades de coma flotante de precisión doble (64-bit)
od-help-f = unidades de coma flotante de precisión simple (32-bit)
od-help-capital-f = unidades de coma flotante de precisión doble (64-bit)
od-error-overflow = Resultado numérico fuera de rango
