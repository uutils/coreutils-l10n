...
cut-after-help =
    Cada llamada debe especificar un modo (qué usar para columnas),
...
cut-help-fields-merged = como -f, pero fusiona los delimitadores adyacentes; el delimitador predeterminado es el espacio en blanco y el delimitador de salida un espacio
...
cut-help-no-partial = con -b, no mostrar caracteres multibyte parciales
# Error messages
...
cut-error-delimiter-only-with-fields = entrada inválida: La opción '--delimiter' ('-d') solo se puede usar al mostrar una secuencia de campos
...
cut-error-field-numbered-from-1 = los campos se numeran desde 1
cut-error-position-numbered-from-1 = las posiciones de byte/carácter se numeran desde 1
cut-error-invalid-field-range = rango de campos no válido
cut-error-invalid-position-range = rango de bytes o caracteres no válido
cut-error-invalid-decreasing-range = rango decreciente no válido
cut-error-invalid-range-no-endpoint = rango no válido sin extremo: { $range }
cut-error-invalid-field-value = valor de campo no válido { $value }
cut-error-invalid-position-value = posición de byte/carácter no válida { $value }
cut-error-field-number-too-large = el número de campo { $value } es demasiado grande
cut-error-position-too-large = el desplazamiento de byte/carácter { $value } es demasiado grande
cut-diag-label-zero-bound = la numeración empieza en 1
cut-diag-help-list-syntax = una lista es N, N-M, N- o -M, separados por comas, como en -f1,4-6,9-
