...
wc-total = total
wc-debug-hw-unavailable = depuração: apoio de hardware indisponível nesta CPU
wc-debug-hw-using = depuração: a usar apoio de hardware (recursos: { $features })
wc-debug-hw-disabled-env = depuração: apoio de hardware desativado pelo ambiente
wc-debug-hw-disabled-glibc = depuração: apoio de hardware desativado por GLIBC_TUNABLES ({ $features })
wc-debug-hw-limited-glibc = depuração: apoio de hardware limitado por GLIBC_TUNABLES (desativado: { $disabled }; ativado: { $enabled })
