...
ls-error-invalid-time-style =
    argument --time-style invalide { $style }
...
ls-help-author =
    Afficher l'auteur en format long. Sur les plateformes prises en charge,
...
ls-total = total { $size }
ls-error-not-directory = impossible d'accéder à { $path } : Ce n'est pas un répertoire
ls-warning-failed-to-get-security-context = impossible d'obtenir le contexte de sécurité de : { $path }
ls-warning-getting-security-context = pendant la récupération du contexte de sécurité de : { $path } : { $error }
smack-error-not-enabled = SMACK n'est pas activé sur ce système
smack-error-label-retrieval-failure = impossible de récupérer le label SMACK : { $error }
smack-error-label-set-failure = impossible de définir le label SMACK de '{ $context }' : { $error }
smack-error-no-label-set = aucun label SMACK défini
