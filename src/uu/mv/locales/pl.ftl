...
mv-prompt-overwrite-mode = zastępuje { $target }, omijając tryb { $mode_info }?
mv-error-cannot-overwrite-non-empty-directory = nie można nadpisać { $target }
mv-error-exchange-two-operands = --exchange wymaga dokładnie dwóch operandów
mv-error-exchange-not-supported = --exchange nie jest obsługiwany na tej platformie
mv-help-exchange = wymienia źródło i miejsce docelowe (atomowo je zamienia)
mv-verbose-exchanged = wymieniono { $from } <-> { $to }
mv-error-dest-appeared = cel { $path } został zastąpiony podczas przenoszenia; odmówiono podążania za nim
