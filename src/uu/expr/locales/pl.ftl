...
expr-usage =
    ...
    expr [OPCJE]
expr-after-help =
    Pusty wiersz poniżej rozdziela rosnące grupy priorytetów.

    WYRAŻENIE może mieć postać:

    - ARG1 | ARG2
          ARG1, jeśli nie jest ani null, ani 0, w przeciwnym razie ARG2

    - ARG1 & ARG2
          ARG1, jeśli żaden z argumentów nie jest null ani 0, w przeciwnym razie 0

    - ARG1 < ARG2
          ARG1 jest mniejsze niż ARG2

    - ARG1 <= ARG2
          ARG1 jest mniejsze lub równe ARG2

    - ARG1 = ARG2
          ARG1 jest równe ARG2

    - ARG1 != ARG2
          ARG1 jest nierówne ARG2

    - ARG1 >= ARG2
          ARG1 jest większe lub równe ARG2

    - ARG1 > ARG2
          ARG1 jest większe niż ARG2
    ...
    - ARG1 % ARG2
          reszta arytmetyczna ARG1 podzielonego przez ARG2

    - NAPIS : WYR_REG
          dopasowanie zakotwiczonego wzorca WYR_REG w NAPISIE

    - match NAPIS WYR_REG
          to samo co NAPIS : WYR_REG

    - substr NAPIS POZ DŁUGOŚĆ
          podciąg NAPIS, POZ liczony od 1

    - index NAPIS  ZNAKI
          indeks w NAPISIE  w którym znaleziono dowolne ZNAKI, lub 0

    - length NAPIS
          długość NAPISU

    - + TOKEN
          interpretuj TOKEN jako ciąg, nawet jeśli jest to słowo kluczowe, takie jak match
    ...
    Błędy są zgłaszane z powtórzeniem wyrażenia i kursorem pod argumentem powodującym błąd,
    gdy stderr jest terminalem; wszystko inne odczytujące stderr otrzymuje zwykły komunikat
    jednowierszowy.
...
expr-diag-help-missing-argument = każdy operator potrzebuje wartości po obu stronach
expr-diag-help-unexpected-argument = powłoka mogła rozszerzyć operator; zapisz go jako '{ "*" }' lub użyj skrótu { "\\*" }
expr-diag-help-non-integer-argument = operatory arytmetyczne wymagają liczb całkowitych; do porównywania ciągów znaków używaj = lub { "!" }=
