...
install-help-strip = usuwa tabele symboli
install-help-strip-program = program służący do usuwania plików binarnych
...
install-help-preserve-context = zachowuje kontekst zabezpieczeń
install-help-context = ustawia kontekst zabezpieczeń plików i katalogów
install-error-dir-needs-arg = { $util_name } z -d wymaga co najmniej jednego argumentu.
install-error-create-dir-failed = nie można utworzyć katalogu { $path }
...
install-error-backup-failed = nie można wykonać kopii zapasowej { $from }: { $error }
install-error-install-failed = nie można zainstalować { $from } do { $to }: { $error }
install-error-strip-failed = program strip nie powiódł się: { $error }
install-error-strip-abnormal = proces strip zakończony nieprawidłowo; kod wyjścia: { $code }
...
install-error-existing-file-not-directory = nie można utworzyć katalogu { $path }: plik istnieje
install-warning-no-strip-with-program = OSTRZEŻENIE: zignorowano opcję --strip-program, ponieważ nie określono opcji -s
install-error-backing-up-destroy-source = tworzenie kopii zapasowej { $dest } może zniszczyć źródło;  { $source } nie zostało skopiowane
